...
def index():
    previous_choice = 'car'  # Default to 'car' if no prior selection
    previous_brand = None

    if request.method == 'POST':
        previous_choice = request.form.get('choice')
        previous_brand = request.form.get('brand_name')
        ...
        if choice == 'car':
            ...
            return render_template('results.html', 
                                   results=recommended_cars.head(5).fillna('-').to_dict('records'), 
                                   all_results=recommended_cars.fillna('-').to_dict('records'), 
                                   type='cars')
            ...
            return render_template('results.html', 
                                   results=recommended_bikes.head(5).fillna('-').to_dict('records'), 
                                   all_results=recommended_bikes.fillna('-').to_dict('records'), 
                                   type='bikes')
